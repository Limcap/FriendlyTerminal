...
namespace Limcap.UTerminal {
	internal partial class Assistant {
		...
		public StringBuilder GetPredictions( string input ) {
			...
			bool LastConfirmedNodeIsNotFinal = !confirmedNode.IsLeafNode;
		...
		private (PString inpCmd, PString inpArgs) SplitInput( string input ) {
			...
			var inpCmd = slicer.NextSlice( PString.Slicer.Mode.IncludeSeparatorAtEnd );
		...
		private static void FixInput( ref PString inpCmd, ref PString inpArgs ) {
			// remove trailing space in the arguments string
			inpArgs.Trim();
			// fix space before colon in the invoke string
			if (inpCmd.len > 1 && inpCmd[inpCmd.len - 1] == ':') {
				while(inpCmd[inpCmd.len - 2] == ' ') {
					inpCmd[inpCmd.len - 2] = ':';
					inpCmd[inpCmd.len - 1] = ' ';
					inpCmd.len--;
				}
		...
		private static (PString, Node) ProcessCommandInput( PString inpCmd, Node initialNode, ref List<Node> result ) {
			...
			bool inputDoesntMatchInvokeString = false;
			result.Clear();
			...
			// a space after it.
			while (slicer.HasNextSlice && !inputDoesntMatchInvokeString) {
				curWord = slicer.NextSlice();
				...
				bool CantFindMatchForWord = !confirmed.FindNext( curWord );
				bool IsLastWord_and_IsIncomplete = !(slicer.HasNextSlice || inpCmd.EndsWith( ':' ));
				
				if (inputDoesntMatchInvokeString = CantFindMatchForWord || IsLastWord_and_IsIncomplete) {
